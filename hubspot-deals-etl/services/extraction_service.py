...
class ExtractionService:
    ...
    def __init__(self, config: Dict[str, Any], source_type: str = "hubspot_deals"):
        ...
        # NEW — HubSpot API Service
        self.hubspot_service = HubSpotAPIService(
            access_token=self.config.get("access_token"),
            base_url=self.config.get("base_url", "https://api.hubapi.com"),
            timeout=30
        )
    ...
    def extract(self, job_id: str):
        ...
        # Start job
        self.job_service.update_job_status(job_id, JobStatus.RUNNING)

        try:
            # Call HubSpot API
            deals = self.hubspot_service.get_all_deals(
                properties=[
            ...
            for deal in deals:
                try:
                    ...
                    # Write batches of 100
                    if len(buffer) >= 100:
                        self.db_service.insert_results("hubspot_deal_results", buffer)
                        buffer = []

                    # Update progress every 50
                    if processed % 50 == 0:
                        self.job_service.update_progress(job_id, processed, failed)

                except Exception as e:
                    failed += 1
                    self.logger.error("Failed to process deal: %s", e)

            # flush remaining
            if buffer:
                self.db_service.insert_results("hubspot_deal_results", buffer)

            self.job_service.mark_completed(job_id, processed, failed)

        except Exception as ex:
            self.logger.error("Scan failed: %s", ex)
            self.job_service.mark_failed(job_id, str(ex))
    ...
    def _setup_dlt_environment(self):
        try:
            dlt_env = build_dlt_env_vars(self.config)
            for k, v in dlt_env.items():
                if not os.environ.get(k):
                    os.environ[k] = v
        except Exception as e:
            self.logger.error("Failed to set up DLT env vars: %s", e)
